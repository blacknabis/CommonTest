...
## [2026-02-22 17:07:40] 후속 작업 진행 2

### 요청
- 커밋 이후 작업 지속 진행 (`SelectionSystem` 프리팹 자산화 단계 실제 반영).
...
- 메뉴 실행으로 프리팹 생성/갱신:
  - `Tools/Kingdom/Build SelectionSystem Prefab` -> `Assets/Resources/UI/SelectionSystem.prefab`
  - `Tools/Kingdom/Build WorldHpBar Prefab` -> `Assets/Resources/UI/WorldHpBar.prefab`
- 런타임 코드 정리:
  - `WorldHpBar`에서 `CanvasGroup` 의존 제거.
...
## [2026-02-22 17:12:58] 후속 작업 진행 3

### 요청
- 추가 진행: 회귀 안정성 확인.

### 반영
- Unity Play Mode에서 `Tools/Kingdom/Run Barracks Melee Smoke Regression` 실행.
- 완료 로그 확인 후 Play Mode 종료.

### 검증
- 콘솔 결과:
  - `[KingdomAppManager] Barracks melee smoke regression finished. success=10, fail=0, spawned=15, killed=2, enemyAttackEvents=30, rallySet=11, maxBlocking=3, maxAssigned=3, maxDead=3, maxRespawning=3, totalDeaths=3, totalRespawns=2`
- 해석:
  - 자동 스모크 기준 배럭 근접전/집결지/리스폰 루프는 회귀 없음.
