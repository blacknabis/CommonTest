...
### Step 2.5 (추가): 배경제거 모드 고도화
목표: 데이터셋 특성에 맞춰 배경 제거 방식을 즉시 전환
- 방식 A: `제거색상 + 허용오차`(Chroma Key)
- 방식 B: `RGB 채널 조건식`(채널별 `>=`, `<=`, `Ignore`)
- 예시 프리셋: `R>=150`, `G<=50`, `B>=150`
- 프리셋 Save/Load와 프리뷰 캐시 해시에 조건식 값 연동

완료 기준:
- 동일 소스에서 두 방식 간 전환 시 결과/프리뷰가 즉시 재현되고,
- 프리셋 로드로 조건식까지 1회에 복원됨

### Step 3 (D6~D8): 단일 소스 액션 분류/출력 정책
목표: 작업 복잡도를 낮추고 입력 실수를 줄인 단일 흐름 유지
- Source Texture 1개 기준 처리
- 액션 그룹 수동 지정 옵션 제공 (`Unknown/Idle/Walk/Attack/Die`)
- 파일명 키워드 자동 분류는 fallback으로만 사용
- 액션 미검출 시 `Unknown` 그룹으로 처리

진행 현황 (2026-02-18):
- 단일 소스 처리 경로로 정리 완료
- 액션 분류 방식은 수동 지정 우선으로 변경
- 자동 분류는 보조 경로로 유지
- 출력 파일명 prefix 반영: `<group>_<원본명>_Processed.png`

완료 기준:
- 단일 Source 입력에서 액션 그룹을 명시적으로 지정해 반복 처리 가능
- 파일명 규칙이 불규칙해도 `Unknown` 포함 안정적으로 출력
