...
## 남은 확인 항목 (수동 QA)
- Unity PlayMode에서 슬라이더/토글 동작 및 재실행 복원 확인
- 프리팹 기반 `UI/WorldMap/AudioOptionsPopup` 리소스 연결 시 경로 자동 바인딩 정상 여부 확인
- 월드맵에서 `btnAudioOptions` 슬롯이 실제 프리팹에 할당되어 있는지 확인
...
3. 남은 확인
- PlayMode에서 월드맵 설정 버튼 클릭 시 폴백이 아닌 프리팹 로드 경로로 열리는지 최종 확인
- 슬라이더/뮤트/재실행 복원 수동 QA
